...
const Homeloan = () => {
  ...
  const phone = useRef();
  const annualincome = useRef();
  const coapplicantincome = useRef();
  const loanamount = useRef();
  const marital = useRef();
  const dependents = useRef();
  const education = useRef();
  const selfemploy = useRef();
  const loanduration = useRef();
  const credithist = useRef();
  const identity = useRef();
  const propertyarea = useRef();

  const router = useRouter();

  const homeLoanSubmitHandler = async (e) => {
    e.preventDefault()
    const homeLoan = {
      ...
      annual_income: annualincome.current.value,
      coapplicant_annual_income: coapplicantincome.current.value,
      loan_amount: loanamount.current.value,
      marital_status: marital.current.value,
      no_of_dependents: dependents.current.value,
      education_status: education.current.value,
      self_employment: selfemploy.current.value,
      loan_duration: loanduration.current.value,
      credit_history: credithist.current.value,
      identity_verified: identity.current.value,
      property_area: propertyarea.current.value,
      bank_name: cookie.bank
    }
    try {
      const res = await axios.post('http://localhost:3001/api/bank/homeloan', homeLoan)
      console.log(res)
      router.push('../status')
    ...
    <div>
      ...
      <div className="flex flex-col items-center justify-center min-h-screen py-2 bg-gray-100">
        ...
        <div className="px-40 py-20 items-center justify-center flex flex-col bg-white rounded-2xl shadow-2xl outline-4 text-violet-700 mb-2">
          <form className="w-full max-w-lg" onSubmit={homeLoanSubmitHandler}>
              ...
              <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option>Not Self-Employed</option>
                  </select>
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                    <svg
                      className="fill-current h-4 w-4"
